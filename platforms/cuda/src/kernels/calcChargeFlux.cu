...
extern "C" __global__ void calcRealCharge(
    ...
    for (int npair = blockIdx.x*blockDim.x+threadIdx.x; npair < NUM_FLUX_BONDS + NUM_FLUX_ANGLES + NUM_FLUX_WATERS; npair += blockDim.x*gridDim.x){
        if (npair < NUM_FLUX_BONDS){
            ...
            atomicAdd(&dqdx_val[pair2].x, v2.x);
            atomicAdd(&dqdx_val[pair3].x, v3.x);
            atomicAdd(&dqdx_val[pair4].x, v4.x);
            atomicAdd(&dqdx_val[pair5].x, v5.x);
            atomicAdd(&dqdx_val[pair6].x, v6.x);
            atomicAdd(&dqdx_val[pair7].x, v7.x);
            atomicAdd(&dqdx_val[pair8].x, v8.x);
            atomicAdd(&dqdx_val[pair9].x, v9.x);
            atomicAdd(&dqdx_val[pair1].y, v1.y);
            atomicAdd(&dqdx_val[pair2].y, v2.y);
            atomicAdd(&dqdx_val[pair3].y, v3.y);
            atomicAdd(&dqdx_val[pair4].y, v4.y);
            atomicAdd(&dqdx_val[pair5].y, v5.y);
            atomicAdd(&dqdx_val[pair6].y, v6.y);
            atomicAdd(&dqdx_val[pair7].y, v7.y);
            atomicAdd(&dqdx_val[pair8].y, v8.y);
            atomicAdd(&dqdx_val[pair9].y, v9.y);
            atomicAdd(&dqdx_val[pair1].z, v1.z);
            atomicAdd(&dqdx_val[pair2].z, v2.z);
            atomicAdd(&dqdx_val[pair3].z, v3.z);
            atomicAdd(&dqdx_val[pair4].z, v4.z);
            atomicAdd(&dqdx_val[pair5].z, v5.z);
            atomicAdd(&dqdx_val[pair6].z, v6.z);
            atomicAdd(&dqdx_val[pair7].z, v7.z);
            atomicAdd(&dqdx_val[pair8].z, v8.z);
            atomicAdd(&dqdx_val[pair9].z, v9.z);
        }
    }
}
...
#endif
        atomicAdd(&forceBuffers[p2], static_cast<unsigned long long>((long long) (- dedq[p1] * dqdx_val[npair].x*0x100000000)));
        atomicAdd(&forceBuffers[p2+PADDED_NUM_ATOMS], static_cast<unsigned long long>((long long) (- dedq[p1] * dqdx_val[npair].y*0x100000000)));
        atomicAdd(&forceBuffers[p2+2*PADDED_NUM_ATOMS], static_cast<unsigned long long>((long long) (- dedq[p1] * dqdx_val[npair].z*0x100000000)));
    }
}
